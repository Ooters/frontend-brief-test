# kindling.net

A small personal homepage, kept by hand. Tasteful early-2000s — handmade but well made.

Plain HTML, CSS, and a teaspoon of JavaScript. No frameworks, no build step, no dependencies. Double-click `index.html` to open it locally, or drop the folder onto any static host.

## Files

- `index.html` — the page
- `styles.css` — the design system
- `site.js` — visitor counter (localStorage) and render-time stat
- `design-briefs/` — the design brief that produced this site

## Hosting

Works on Neocities, GitHub Pages, Netlify Drop, Cloudflare Pages, Vercel, Surge — anywhere that serves static files.
